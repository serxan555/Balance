 ...
 * @author SS555
 */
public class GelirEtrafliHesabatController implements Initializable {
    ...
    public void cemBalance() {
        cemBalans.setText("Ümumi Balans: " + String.valueOf(balanceRepository.sum("income")));
    }
    ...
    private void filterEt(ActionEvent event) {
        ...
        List<Object> list = balanceRepository.filter(basla,son, "income");
        cedvel.getItems().setAll(list);
        cemBalans.setText("Ümumi Balans: " + String.valueOf(balanceRepository.sumFilter(basla,son,"income")));
    ...
    private void fillTable() {
        List<Object> list = balanceRepository.findAll("income");
        cedvel.getItems().setAll(list);
        Notification.callNotification(String.valueOf(list.size()) + " Sətir" , "BİLDİRİŞ!!!");
    }
